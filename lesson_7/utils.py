def convert_data(data_str):
    ...
    total_height = sum([float(data_list[idx]) for idx in range(1, len(data_list), 3)]) * 2.54
    total_weight = sum([float(data_list[idx]) for idx in range(2, len(data_list), 3)]) * 0.453592
